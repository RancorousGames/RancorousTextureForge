...
import { AppState, GridSettings, TextureAsset } from '../types';
...
export function useAutoDetect(
  ...
  const handleAutoDetectMainGrid = useCallback(async (asset?: TextureAsset) => {
    const totalStart = performance.now();
    let imageData: ImageData | null = null;

    if (asset) {
      const loadStart = performance.now();
      const img = await loadImage(asset.sourceUrl || asset.url).catch(() => null);
      if (img) {
    ...
    if (!imageData) {
      ...
      const sharedSourceUrls = Array.from(new Set(
        state.atlasEntries.map(t => t.sourceUrl).filter((u): u is string => Boolean(u))
      ));
  ...
  }, [state.atlasEntries, state.gridSettings, canvasWidth, canvasHeight, set, onSettingsDetected]);

  const handleAutoDetectSourceGrid = useCallback(async (sourceAsset: TextureAsset) => {
    const totalStart = performance.now();
    const loadStart = performance.now();
    const img = await loadImage(sourceAsset.sourceUrl || sourceAsset.url).catch(() => null);
    if (!img) return;
